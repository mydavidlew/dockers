...
if [[ $# -eq 2 && $1 == $C_RUN && $2 == $APP_DS ]] ; then
  ...
  $C_DOCKER --hostname=$D_PA1.local --name=$D_PA1 --network=$D_NETWORK -v /var/run/docker.sock:/var/run/docker.sock -v /var/lib/docker/volumes:/var/lib/docker/volumes -p 9001:9001 portainer/agent:latest
