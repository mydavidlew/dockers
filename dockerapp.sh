...
if [[ $# -eq 2 && $1 == $C_RUN && $2 == $APP_DS ]] ; then
  ...
  $C_DOCKER --hostname=$D_ES1.local --name=$D_ES1 --network=$D_NETWORK $C_MOUNT,src=$L_PATH"/elasticsearch/conf",dst=/usr/share/elasticsearch/config --env "discovery.type=single-node" -p 9200:9200 -p 9300:9300 docker.elastic.co/elasticsearch/elasticsearch:7.12.0
  sleep $SLEEP_INT
  $C_DOCKER --hostname=$D_KI1.local --name=$D_KI1 --network=$D_NETWORK $C_MOUNT,src=$L_PATH"/kibana/conf",dst=/usr/share/kibana/config --link $D_ES1:$L_ES1 -p 5601:5601 docker.elastic.co/kibana/kibana:7.12.0
  sleep $SLEEP_INT
  $C_DOCKER --hostname=$D_LS1.local --name=$D_LS1 --network=$D_NETWORK $C_MOUNT,src=$L_PATH"/logstash/conf",dst=/usr/share/logstash/config $C_MOUNT,src=$L_PATH"/logstash/pipeline",dst=/usr/share/logstash/pipeline --link $D_ES1:$L_ES1 -p 5044:5044 -p 9600:9600 docker.elastic.co/logstash/logstash:7.12.0
  sleep $SLEEP_INT
elif [[ $# -eq 2 && $1 != $C_RUN && $2 == $APP_ES ]] ; then
  check_network
  check_command $1
  ...
  $C_DOCKER --hostname=$D_AB1.local --name=$D_AB1 --network=$D_NETWORK $C_MOUNT,src=$L_PATH"/allbeats/conf/auditbeat.yml",dst=/usr/share/auditbeat/auditbeat.yml --cap-add="AUDIT_CONTROL" --link $D_ES1:$L_ES1 --link $D_KI1:$L_KI1 --link $D_LS1:$L_LS1 docker.elastic.co/beats/auditbeat:7.12.0
  sleep $SLEEP_INT
  $C_DOCKER --hostname=$D_FB1.local --name=$D_FB1 --network=$D_NETWORK $C_MOUNT,src=$L_PATH"/allbeats/conf/filebeat.yml",dst=/usr/share/filebeat/filebeat.yml --link $D_ES1:$L_ES1 --link $D_KI1:$L_KI1 --link $D_LS1:$L_LS1 docker.elastic.co/beats/filebeat:7.12.0
  sleep $SLEEP_INT
  $C_DOCKER --hostname=$D_HB1.local --name=$D_HB1 --network=$D_NETWORK $C_MOUNT,src=$L_PATH"/allbeats/conf/heartbeat.yml",dst=/usr/share/heartbeat/heartbeat.yml --link $D_ES1:$L_ES1 --link $D_KI1:$L_KI1 --link $D_LS1:$L_LS1 docker.elastic.co/beats/heartbeat:7.12.0
  sleep $SLEEP_INT
  $C_DOCKER --hostname=$D_JB1.local --name=$D_JB1 --network=$D_NETWORK $C_MOUNT,src=$L_PATH"/allbeats/conf/journalbeat.yml",dst=/usr/share/journalbeat/journalbeat.yml --link $D_ES1:$L_ES1 --link $D_KI1:$L_KI1 --link $D_LS1:$L_LS1 docker.elastic.co/beats/journalbeat:7.12.0
  sleep $SLEEP_INT
  $C_DOCKER --hostname=$D_MB1.local --name=$D_MB1 --network=$D_NETWORK $C_MOUNT,src=$L_PATH"/allbeats/conf/metricbeat.yml",dst=/usr/share/metricbeat/metricbeat.yml --link $D_ES1:$L_ES1 --link $D_KI1:$L_KI1 --link $D_LS1:$L_LS1 docker.elastic.co/beats/metricbeat:7.12.0
  sleep $SLEEP_INT
  $C_DOCKER --hostname=$D_PB1.local --name=$D_PB1 --network=$D_NETWORK $C_MOUNT,src=$L_PATH"/allbeats/conf/packetbeat.yml",dst=/usr/share/packetbeat/packetbeat.yml --cap-add=NET_ADMIN --link $D_ES1:$L_ES1 --link $D_KI1:$L_KI1 --link $D_LS1:$L_LS1 docker.elastic.co/beats/packetbeat:7.12.0
  sleep $SLEEP_INT
elif [[ $# -eq 2 && $1 != $C_RUN && $2 == $APP_EB ]] ; then
  check_network
  check_command $1
  ...
  $C_DOCKER --hostname=$D_PA1.local --name=$D_PA1 --network=$D_NETWORK --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v /var/lib/docker/volumes:/var/lib/docker/volumes -p 9001:9001 portainer/agent:latest
  sleep $SLEEP_INT
elif [[ $# -eq 2 && $1 != $C_RUN && $2 == $APP_PA ]] ; then
  check_network
  check_command $1
