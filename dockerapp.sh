...
if [[ $# -eq 0 ]] ; then
    ...
    if [[ $# -eq 1 && $1 == $CMD_START_DS ]] ; then
        ...
        $C_DOCKER --name=$D_MY1 --network=$D_NETWORK --env "MYSQL_ROOT_PASSWORD=password" $C_MOUNT,src=$L_PATH"/mysql/etc/my.cnf",dst=/etc/my.cnf $C_MOUNT,src=$L_PATH"/mysql/data",dst=/var/lib/mysql -p 3306:3306 mysql/mysql-server
        sleep $SLEEP_INT
