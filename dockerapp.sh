...
#!/bin/bash
#[Docker-Final]
CMD_START_DS=start-ds; CMD_STOP_DS=stop-ds; CMD_RUN_DS=run-ds
CMD_START_MA=start-ma; CMD_STOP_MA=stop-ma; CMD_RUN_MA=run-ma
CMD_START_MS=start-ms; CMD_STOP_MS=stop-ms; CMD_RUN_MS=run-ms
CMD_START_ES=start-es; CMD_STOP_ES=stop-es; CMD_RUN_ES=run-es
CMD_START_EB=start-eb; CMD_STOP_EB=stop-eb; CMD_RUN_EB=run-eb
CMD_START_DE=start-de; CMD_STOP_DE=stop-de; CMD_RUN_DE=run-de
...
SLEEP_INT=1
D_NETWORK=appnet
C_DOCKER01="docker run --rm -d"
C_DOCKER02="docker run -d"
C_DOCKER=$C_DOCKER02
...
if [[ $# -eq 0 ]] ; then
    ...
    if [[ $# -eq 1 && $1 == $CMD_START_DS ]] ; then
        ...
        sleep $SLEEP_INT
    elif [[ $# -eq 1 && $1 == $CMD_RUN_DS ]] ; then
        echo "$(date) $line $$: running all datastore services"
        docker start $D_MY1 $D_PG1 $D_MG1 $D_RD1 $D_SL1
        sleep $SLEEP_INT
        ...
        sleep $SLEEP_INT
    elif [[ $# -eq 1 && $1 == $CMD_RUN_MA ]] ; then
        echo "$(date) $line $$: running all monitor agent services"
        docker start $D_CE1 $D_NE1 $D_CA1
        sleep $SLEEP_INT
        ...
        sleep $SLEEP_INT
    elif [[ $# -eq 1 && $1 == $CMD_RUN_MS ]] ; then
        echo "$(date) $line $$: running all monitor server services"
        docker start $D_AM1 $D_PR1 $D_GR1
        sleep $SLEEP_INT
        ...
        sleep $SLEEP_INT
    elif [[ $# -eq 1 && $1 == $CMD_RUN_ES ]] ; then
        echo "$(date) $line $$: running all elastic stack services"
        docker start $D_ES1 $D_KI1 $D_LS1
        sleep $SLEEP_INT
        ...
        sleep $SLEEP_INT
    elif [[ $# -eq 1 && $1 == $CMD_RUN_EB ]] ; then
        echo "$(date) $line $$: running all elastic beats services"
        docker start $D_AB1 $D_FB1 $D_HB1 $D_JB1 $D_MB1 $D_PB1
        sleep $SLEEP_INT
        ...
        sleep $SLEEP_INT
    elif [[ $# -eq 1 && $1 == $CMD_RUN_DE ]] ; then
        echo "$(date) $line $$: running all drools engine services"
        docker start $D_BC1 $D_KS1
        sleep $SLEEP_INT
        ...
        docker stop $D_XX1
        sleep $SLEEP_INT
    elif [[ $# -eq 1 && $1 == $CMD_RUN_XX ]] ; then
        echo "$(date) $line $$: running all XXX services"
        docker start $D_XX1
        sleep $SLEEP_INT
    else
